<template>
  <div>
    <canvas :id="options.id" width="100" height="100"></canvas>
  </div>
</template>
<script>
  export default{
    ...
    methods: {
      drawCircle(_options){
        ...
        options.color = options.color || '#4da1ff';    //定义默认颜色（包括字体和边框颜色）;
        options.lineWidth = options.lineWidth || 10;    //定义默认圆描边的宽度;
        options.lineCap = options.lineCap || 'round';    //定义描边的样式，square为直角边，round 为圆角
        ...
        ctx.fillText((options.angle * 100) + '%', sCenter+5, sCenter+5);    //设置填充文字;
