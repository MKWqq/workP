        ...
        <el-table :data="classesTimeSetTable" @cell-click="tableCellClick" border class="g-timeSettingTable">
          ...
          <el-table-column v-for="n in 7" :key="n" :label="weekData[n-1]">
            <template scope="props">
              ...
              <span v-if="props.row[n-1].statu==2 || props.row[n-1].statu==3 || props.row[n-1].statu==4" class="NotCourse">不排课</span>
    ...
    methods:{
          ...
          if(row[columnNum].statu==0){
            ...
            if(row[columnNum].statu==5){
              if(row[columnNum].subject==this.curriculumName){
                ...
                this.$alert('已设置预先安排'+row[columnNum].subject+'！','提示',{
                  confirmButtonText:'确定',
                  type:'error',
                  callback:action=>{}
                });
      ...
      getTableData(){
        coursePreviousTableLoad({pkListId:this.pkListId,gradeId:this.gradeId,classId:this.classId,subjectId:this.curriculumId}).then(data=>{
          ...
          this.teacherName=this.handlerData(data).teacherName;
          this.teacherId=this.handlerData(data).techerId;
          console.log(data);
        });
      },
      /*处理数据*/
